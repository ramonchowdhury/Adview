      ...
      <div class="card-columns">
        ...
        if(isset($_GET["setpage"])){
          ...
          $start = 0;
				...
				$query = "SELECT * FROM `ads` WHERE `category` = '$getCategory' LIMIT $start,$limit";
				$query_run = mysqli_query($con,$query);
	        ...
	        <div class="card bg-secondary">
	            <a href="single.php?id=<?php echo $result['id']?>" class="my-image" ><img class="img-responsive img-fluid" src="<?php echo $result['directory']?>/<?php echo $result['image1']?>" alt=""></a>
	          <div class="card-body effect">
	            <h4 class="card-title bg-dark p-2 rounded text-center text-light text-uppercase"><?php echo $result['title']?></h4>
	            <p class="my-card-body card-body text-justify"><?php echo textShorten($result['discription']); ?></p>
	            <a href="single.php?id=<?php echo $result['id']?>" class="btn btn-warning btn-block btn-lg">Details</a>
	          </div>
	        </div>
		<?php
			  }
		 ...
		 ?>
            ...
            <nav class="mt-5 pt-3">
              <ul class="pagination justify-content-center">
                  <?php
                    $query = "SELECT * FROM `ads` WHERE `category` = '$getCategory'";
                    ...
                    if($count > 0)
                      for($i=1;$i<=$pageno;$i++){
                        if(isset($_GET["setpage"])){
                          if($_GET['setpage']==$i){
                            echo "<li class='page-item'><a href='category.php?category=".$getCategory."&setpage=".$i."' class ='active page-link'>$i</a></li> ";

                          }
                          else{
                            echo "<li class='page-item'><a href='category.php?category=".$getCategory."&setpage=".$i."' class='page-link'>$i</a></li> ";
                          }
                        }
                        else{
                          echo "<li class='page-item'><a href='category.php?category=".$getCategory."&setpage=".$i."' class='page-link'>$i</a></li> ";
